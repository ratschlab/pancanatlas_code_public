...
def run_tumor_normal_coloring(tn_plot_dir, metadata_df, pca_model, pca_embeds, tsne_embeds_dict):
#    cmap = dict(zip([True, False], eplt.sns.color_palette("Set2", 2)))
    ...
    if do_tsne:
        for (pp, lr), tsne_embeds in tsne_embeds_dict.items():
            ...
            fig.suptitle('Expression tSNE(perplexity=%d, learning_rate=%d)' %(pp, lr))
...
do_hl_embeddings = True
do_tumor_normal = True


if __name__ == '__main__':
    EMBED_DIR = os.path.join(config.embed_dir, 'expression')
    PLOT_DIR = os.path.join(config.plot_dir, 'expression', 'embeds')
    assert os.path.exists(EMBED_DIR)
    if not os.path.exists(PLOT_DIR): os.makedirs(PLOT_DIR)
    print "EMBED_DIR: %s" %EMBED_DIR
    print "PLOT_DIR: %s" %PLOT_DIR
    ...
    if do_tumor_normal:
        tn_plot_dir = os.path.join(PLOT_DIR, 'tumor_normal')
        run_tumor_normal_coloring(tn_plot_dir, metadata_df, pca_model, pca_embeds, tsne_embeds_dict)

    if do_regular_embeddings:
        plotting_df, legend_kwargs, default_kwargs = \
            eplt.load_cnc_color_tumor_marker_kwargs(metadata_df)
        reg_plot_dir = os.path.join(PLOT_DIR, 'complete')
