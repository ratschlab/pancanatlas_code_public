...
tissues = [('BLCA', 'BLDR'), ('BRCA', 'BRST'), ('COAD', 'COLN'), ('HNSC', 'SKIN'), ('KICH', 'KIDN'), ('KIRC', 'KIDN'), ('LIHC', 'LIVR'), ('LUAD', 'LUNG'), ('LUSC', 'LUNG'), ('PRAD', 'PRST'), ('READ', 'COLN'), ('THCA', 'THYR'), ('UCEC', 'UTER')]
...
nan_t = 5 ### this is the number of non-NaN samples per tissue type for an event to be counted
...
for event_type in event_types:
    ...
    anno_file = os.path.join(paths.basedir_as_sub, 'merge_graphs_%s_C%i.anno_only.pickle' % (event_type, CONF))
    anno_idx = cPickle.load(open(anno_file, 'r'))
    non_anno_idx = sp.where(~anno_idx)[0]
    anno_idx = sp.where(anno_idx)[0]

    if not os.path.exists(os.path.join(paths.basedir_as_sub, 'comparisons', 'pickle')):
        os.makedirs(os.path.join(paths.basedir_as_sub, 'comparisons', 'pickle'))
    count_pickle = os.path.join(paths.basedir_as_sub, 'comparisons', 'pickle', 'sample_psi_counts.%s.conf_%i.subsampled.pickle' % (event_type, nan_t))
    ...
    if os.path.exists(count_pickle) and not '--force' in sys.argv:
        ...
        hdf5_files = {'tc' : os.path.join(paths.basedir_as_sub, 'merge_graphs_%s_C%i.counts.hdf5' % (event_type, CONF)),
                      'tn' : os.path.join(paths.basedir_as_sub, 'merge_graphs_%s_C%i.counts.hdf5' % (event_type, CONF))} #,
        ...
        for t in dsets:
            if t == 'gt':
                ...
                ### load libsize
                libsize = sp.loadtxt(paths.libsize_sub, dtype='str', delimiter='\t')
                libsize = libsize[1:, :]
        ...
        for i, p in enumerate(tissues):
            ### get size of normal dataset
            subsample = 10
            #subsample = sp.sum((tids['tn'] == p[0]) & ~is_tumor)
            if p[0] == 'ALL':
                subsample = sp.sum(~is_tumor)
            ### get indices
            for j, t in enumerate(dsets):
                if p[0] == 'ALL':
                    ...
                    if t == 'gt':
                        ...
                        t_idx[(t, p)] = sp.where((tids[t] == p[0]) & ~is_tumor)[0]
                ### subsample
                if t_idx[(t, p)].shape[0] > subsample:
                    t_idx[(t, p)] = sp.sort(npr.choice(t_idx[(t, p)], size=subsample, replace=False))
            ...
            for cc, chunk in enumerate(range(0, hdf5_handles[t]['psi'].shape[1], chunksize)):
                ...
                for mr in mr_t:
                    for i, p in enumerate(tissues):
                        ...
                        for dp in d_psi_t:
                            if cc == 0:
                                count[(t, p)][(mr, dp)] = tmp_idx[(sp.sum(idx_nn, axis=0) >= min(nan_t, subsample)) & (d_psi >= dp)]
                            else:
                                count[(t, p)][(mr, dp)] = sp.r_[count[(t, p)][(mr, dp)], tmp_idx[(sp.sum(idx_nn, axis=0) >= min(nan_t, subsample)) & (d_psi >= dp)]]
    ...
    for mr in mr_t:
            ...
            for p in tissues:
                ...
                for i, t in enumerate(dsets_plot):
                    if count[(t, p)] == 0:
                        ...
                        counts.append(sp.intersect1d(anno_idx, count[(t, p)][(mr, thresh)]).shape[0])
                    curr_xloc.append(j*len(dsets_plot) + i + buff)
                    #labels.append(label_dict[t][p[0]] + '\nN=%i' % t_idx[(t, p)].shape[0])
                    labels.append(label_dict[t][p[0]])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(PLOTDIR, 'dpsi_event_overview.%s%s.subset.pdf' % (event_type, plot_tag)), format='pdf')
        plt.savefig(os.path.join(PLOTDIR, 'dpsi_event_overview.%s%s.subset.png' % (event_type, plot_tag)), format='png')
        plt.close(fig)
