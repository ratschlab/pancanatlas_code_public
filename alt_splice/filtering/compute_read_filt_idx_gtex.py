...
### load size factors
libsize = sp.loadtxt(paths.libsize_gtex, dtype='str', delimiter='\t')
libsize = libsize[1:, :]
...
for et in event_types:
    ...
    IN = h5py.File(os.path.join(paths.basedir_as_gtex, 'merge_graphs_%s_C%i.counts.hdf5' % (et, CONF)), 'r')
    c = IN['iso1'].chunks
    ...
    OUT = h5py.File(os.path.join(paths.basedir_as_gtex, 'merge_graphs_%s_C%i.min_read_filt.hdf5' % (et, CONF)), 'w')
