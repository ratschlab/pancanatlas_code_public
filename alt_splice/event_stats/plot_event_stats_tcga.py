...
def main():
    ...
    for e, event_type in enumerate(event_types):
        ...
        idx_psi_icgc = cPickle.load(open(os.path.join(paths.basedir_as, 'merge_graphs_%s_C%i.counts.psi_filt_per_ct.pickle' % (event_type, CONF)), 'r'))[1]
        ...
        for dp in [0.0, 0.1, 0.3, 0.5]:
            for mr in [5, 20, 50]:
                ...
                if  event_type == 'mutex_exons':
                    ...
                    counts_anno = sp.array([sp.sum((idx_conf_tcga & is_anno_icgc)[idx_psi_icgc[(mr, ct, dp)]]) if (mr, ct, dp) in idx_psi_icgc else 0 for ct in ctypes])
                    counts_new = sp.array([sp.sum(idx_conf_tcga[idx_psi_icgc[(mr, ct, dp)]]) - sp.sum((idx_conf_tcga & is_anno_icgc)[idx_psi_icgc[(mr, ct, dp)]]) if (mr, ct, dp) in idx_psi_icgc else 0 for ct in ctypes])
