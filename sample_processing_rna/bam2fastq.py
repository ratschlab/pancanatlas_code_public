...
if len(sys.argv) < 2:
    print >> sys.stderr, 'Usage: %s <bam_file> [<max_len>]' % sys.argv[0]
    sys.exit(1)
fname = sys.argv[1]
maxlen = 100000
if len(sys.argv) > 2:
...
def _print(curr_r1, curr_r2):
    ...
    global out_r2
    global maxlen
    print >> out_r1, '@' + curr_r1[0] + '/1'
    print >> out_r1, curr_r1[2][:maxlen]
    print >> out_r1, '+'
    print >> out_r1, curr_r1[1][:maxlen]
    print >> out_r2, '@' + curr_r2[0] + '/2'
    print >> out_r2, curr_r2[2][:maxlen]
    print >> out_r2, '+'
    print >> out_r2, curr_r2[1][:maxlen]
...
for read in samfile.fetch(until_eof=True):
    if cnt > 0 and cnt % 100000 == 0:
        print >> sys.stderr, '[processed %.2f million read pairs]' % (cnt / 1000000.0)
    curr_id = read.qname
